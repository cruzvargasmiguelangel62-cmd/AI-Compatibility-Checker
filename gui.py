...
class App(ctk.CTk):
    def __init__(self):
        ...
        # Cross-platform font and DPI scaling adjustments
        font_family = "Segoe UI" if platform.system() == "Windows" else "DejaVu Sans"
        try:
            tkfont.nametofont("TkDefaultFont").configure(family=font_family, size=12)
            tkfont.nametofont("TkTextFont").configure(family=font_family, size=12)
            tkfont.nametofont("TkMenuFont").configure(family=font_family, size=11)
        except Exception:
            pass

        # Allow override via environment variable UI_SCALE (e.g. 1.2)
        try:
            ui_scale = float(os.environ.get("UI_SCALE", "0"))
        except Exception:
            ui_scale = 0
        if ui_scale > 0:
            self.tk.call('tk', 'scaling', ui_scale)
        else:
            # Apply a modest upscaling on Linux for better readability on varied GTK themes/DPI
    ...
    def _bind_mousewheel_to_frame(self, frame):
        """Bind mousewheel/touchpad scroll to a scrollable frame for Linux, macOS, Windows compatibility."""
        def _on_mousewheel(event):
            # event.delta on Windows/macOS, event.num on Linux (Button-4 up, Button-5 down)
            # Scroll up (Linux: event.num==4, Windows/macOS: delta > 0)
            if event.num == 4 or event.delta > 0:
                frame._parent_canvas.yview_scroll(-5, "units")
            # Scroll down (Linux: event.num==5, Windows/macOS: delta < 0)
        ...
        # Bind for Windows/macOS (MouseWheel)
        frame.bind("<MouseWheel>", _on_mousewheel, add="+")
        # Also bind on the parent canvas for better responsiveness
        try:
    ...
    def create_layout(self):
        ...
        self.grid_columnconfigure(0, weight=0, minsize=380)  # Sidebar specs (wider on Linux)
        ...
        # Enable touchpad scroll on sidebar
    ...
    def build_model_list_section(self):
        ...
        self.guide_frame.grid(row=4, column=0, sticky="ew", pady=(12, 0))
        self.guide_frame.grid_columnconfigure(0, weight=1)
        self.guide_frame.grid_propagate(False)

        guide_text = (
            "💡 ¿Cómo ejecutar estos modelos? 1. Instala Ollama (para LLMs) o LM Studio. "
            "2. Abre tu terminal y escribe: 'ollama run [nombre-modelo]' (ej: 'ollama run deepseek-r1:8b'). "
            "3. Para generar imágenes, usa Stable Diffusion WebUI, ComfyUI o Draw Things."
        )
        ...
        self.guide_label.grid(row=0, column=0, sticky="w", padx=12, pady=12)
